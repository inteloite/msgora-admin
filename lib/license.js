...
const LICENSE_SECRET = process.env.LICENSE_SECRET || 'Msg@9nPw4yBz7rKe3vXt1oHs8cUfLd5jNq2mWgA6iY';

// Key format: MSG-{expiryHex8}-{hmac6+deviceHex2}-{hmac8}-{hmac8}
export function generateKey({ machineId, expiryTs, deviceLimit }) {
    ...
    const payload = `MSG|${machineId.toUpperCase()}|${expiryHex}|${deviceHex}`;
    const hmac = crypto
        .createHmac('sha256', LICENSE_SECRET)
        .update(payload)
        .digest('hex')
        .toUpperCase();
    const p2 = hmac.slice(0, 6) + deviceHex;  // hmac-first, device-last
    const p3 = hmac.slice(6, 14);
    const p4 = hmac.slice(14, 22);
    return `MSG-${expiryHex}-${p2}-${p3}-${p4}`;
}
...
export function validateKey(key, machineId) {
    ...
    const expiryHex = p1;
    const deviceHex = p2.slice(6, 8);  // device-last in p2
    const hmacGiven = (p2.slice(0, 6) + p3 + p4).toUpperCase();
    const payload = `MSG|${machineId.toUpperCase()}|${expiryHex}|${deviceHex}`;
